...
public:
    ...
    QTester(int n, int numOps, double Lambda, double rndLt) : n(n), numOps(numOps), Lambda(Lambda), rndLt(rndLt), qObj() {
        ...
        EnqthrTimes.resize(n, 0) ;
        DeqthrTimes.resize(n, 0) ;
    }

    void enqueuer(){            //This function is only for enqueque
        random_device rd;
        mt19937 generator(rd());
        uniform_int_distribution<int> distribution(1, 100);
        T* v;
        v = new T(distribution(generator));
        qObj.enq(v);
    }
...
int main() {
    ...
    std::vector<thread> enq_thread(2) ; //These two threads always enqueue

    for (int i = 0; i < 2; i++) {
        enq_thread[i] = thread([&tester, i]() {
            tester.enqueuer();
        });
    }


    log_file=fopen("NLQ_nondeterministic","w");
