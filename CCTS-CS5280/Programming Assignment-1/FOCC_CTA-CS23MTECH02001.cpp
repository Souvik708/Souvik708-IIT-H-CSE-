...
FOCC_CTA::FOCC_CTA(int N){                      //Constructor
    this->no_dataitems=N ;
}
...
int FOCC_CTA::read(int tid, int index, int * buffer){
...
int FOCC_CTA::write(int tid, int index, int value){
...
// "tryc" Function is envoked when a transaction is trying to commit

string FOCC_CTA::tryC(const int tid) {
    // Reset the set of conflicting transactions
    conflict_transaction.clear();

    // Check for conflicts with read transactions
    checkConflicts(*transaction[tid]);

    // Remove the current transaction from the set of conflicting transactions
    conflict_transaction.erase(tid);

    if (!conflict_transaction.empty()) {
        // Abort the "current transaction" if there are conflicting transactions
        transaction[tid]->setStatus("abort");
        for (auto e : transaction[tid]->writeset) {
            dataitem[e]->datalock.lock() ;
            dataitem[e]->readlist.erase(transaction[tid]);
        }
        return "abort";
    }
    else {
        // Commit the "current transaction" if there are no conflicts
        transaction[tid]->setStatus("commit");
        for (auto e : transaction[tid]->writeset) {
            dataitem[e]->datalock.lock() ;
            dataitem[e]->vaule = transaction[tid]->LocalBuffer[e];
        }
        return "commit";
    }
}

void FOCC_CTA::checkConflicts(const Transaction& tx) {
    for (auto e : tx.writeset) {
        auto& item = *dataitem[e];
        std::lock_guard<std::mutex> lock(item.datalock);
        for (auto g : item.readlist) {
            if (g->getStatus() == "live") {
                conflict_transaction.insert(g->transaction_Id);
            }
        }
    }
}
...
void update_mem(FOCC_CTA * focc, int numTrans, int tid) ;
...
int main(){
    ...
    FOCC_CTA* focc_ota= new FOCC_CTA(m) ;
    ...
    for(int i=0 ; i< n ; i++){
        t_id[i]=thread(&update_mem,focc_ota,n,i) ;
    }
    for(int i=0 ; i< n ; i++){
        t_id->join() ;
    }
...
void update_mem(FOCC_CTA * focc, int numTrans, int tid){
    ...
    for(int curTrans=0 ; curTrans<numTrans ; curTrans++){
        ...
        do{
            int id= focc->FOCC_CTA::begin_trans() ;
